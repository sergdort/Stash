...
export async function saveItem(db: Db, input: SaveItemInput): Promise<SaveItemResult> {
...
export function listItems(db: Db, input: ListItemsInput): ListItemsResult {
...
export function getItem(db: Db, itemId: number): SaveItemResult["item"] | undefined {
  const row = getItemRowById(db, itemId)
  if (!row) {
    return undefined
  }
